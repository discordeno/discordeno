...
const results = JSON.parse(await Deno.readTextFile("./output.txt"));
...
if (window.BENCHMARK_DATA) {
  //@ts-ignore
  const benchmarks = JSON.parse(JSON.stringify(window.BENCHMARK_DATA.entries.Benchmark)).slice(-1)[0] as {
    commit: {
      author: { email: string; name: string; username: string };
      committer: { email: string; name: string; username: string };
      distinct: boolean;
  ...
  for (const benchmark of benchmarks.benches) {
    compare[benchmark.name] = {
      //@ts-ignore
      previous: {},
      ...compare[benchmark.name],
