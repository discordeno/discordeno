	...
	emojis: data.emojis.map(emoji => createEmoji(emoji)),
		...
		createChannel: (name: string, options?: ChannelCreateOptions) => {
			...
			return client.RequestManager.post(endpoints.GUILD_CHANNELS(data.id), {
				...
				permission_overwrites: options?.permission_overwrites ? options.permission_overwrites.map(perm => ({ allow: perm.allow.map(p => Permissions[p]), deny: perm.deny.map(p => Permissions[p]), ...perm })) : undefined,
				...options
			})
		}
	}
