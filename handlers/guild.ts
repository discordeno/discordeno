...
import { CreateChannelOptions, ChannelCreatePayload, ChannelTypes } from "../types/channel.ts"
import { ImageFormats, ImageSize } from "../types/cdn.ts"
import { CreateEmojisOptions, PositionSwap, EditEmojisOptions, CreateRoleOptions, PrunePayload, FetchMembersOptions, GetAuditLogsOptions, EditIntegrationOptions, BanOptions, GuildEditOptions } from "../types/guild.ts"
...
export function guildIconURL(guild: Guild, size: ImageSize = 128, format?: ImageFormats)
{
	return guild.icon
...
export function guildSplashURL(guild: Guild, size: ImageSize = 128, format?: ImageFormats)
{
	return guild.splash
	? formatImageURL(
...
export function guildBannerURL(guild: Guild, size: ImageSize = 128, format?: ImageFormats)
{
	return guild.banner
	? formatImageURL(
...
export function swapChannels(guildID: string, channelPositions: PositionSwap[]) {
...
export function editEmoji(guildID: string, id: string, options: EditEmojisOptions) {
if (
	!botHasPermission(guildID, [Permissions.MANAGE_EMOJIS])
) {
	throw new Error(Errors.MISSING_MANAGE_EMOJIS);
...
export async function createGuildRole(guild: Guild, options: CreateRoleOptions, reason?: string) {
if (
...
export function editRole(guildID: string, id: string, options: CreateRoleOptions) {
if (
...
export function editEmbed(guildID: string, enabled: boolean, channelID?: string | null) {
if (
	!botHasPermission(guildID, [Permissions.MANAGE_GUILD])
) {
	throw new Error(Errors.MISSING_MANAGE_GUILD);
...
export function editIntegration(guildID: string, id: string, options: EditIntegrationOptions) {
if (
	!botHasPermission(guildID, [Permissions.MANAGE_GUILD])
) {
	throw new Error(Errors.MISSING_MANAGE_GUILD);
