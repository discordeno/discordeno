...
export function transformAuditLogEntryToDiscordAuditLogEntry (
  ...
  return {
        ...
        case '$remove':
          return {
            ...
            new_value: (
              ...
              id: val.id
                ? client.transformers.reverse.snowflake(val.id)
                : undefined,
              name: val.name
            })),
            old_value: (
        ...
        case 'id':
          return {
            ...
            old_value: change.old
              ? client.transformers.reverse.snowflake(change.old as bigint)
              : '',
            new_value: change.new
              ? client.transformers.reverse.snowflake(change.new as bigint)
              : ''
          }
        case 'name':
        case 'description':
    ...
    user_id: payload.userId
      ? client.transformers.reverse.snowflake(payload.userId)
      : null,
    target_id: payload.targetId
      ? client.transformers.reverse.snowflake(payload.targetId)
      : null,
    ...
    options: payload.options
      ? {
          ...
          channel_id: payload.options.channelId
            ? client.transformers.reverse.snowflake(payload.options.channelId)
            : '',
          message_id: payload.options.messageId
            ? client.transformers.reverse.snowflake(payload.options.messageId)
            : '',
          count:
            payload.options.count === 0 ? payload.options.count.toString() : '',
          id: payload.options.id
            ? client.transformers.reverse.snowflake(payload.options.id)
            : '',
