...
export function transformSubscription(bot: Bot, payload: DiscordSubscription): Subscription {
  ...
  if (props.skuIds && payload.sku_ids) subscription.skuIds = payload.sku_ids.map((x) => bot.transformers.snowflake(x))
  if (props.entitlementIds && payload.entitlement_ids) subscription.entitlementIds = payload.entitlement_ids.map((x) => bot.transformers.snowflake(x))
