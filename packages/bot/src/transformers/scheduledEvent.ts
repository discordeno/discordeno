...
export function transformScheduledEvent(bot: Bot, payload: DiscordScheduledEvent) {
  ...
  if (props.id) scheduledEvent.id = bot.transformers.snowflake(payload.id)
  if (props.guildId) scheduledEvent.guildId = bot.transformers.snowflake(payload.guild_id)
  ...
  if (props.scheduledStartTime) scheduledEvent.scheduledStartTime = Date.parse(payload.scheduled_start_time)
  ...
  if (props.name) scheduledEvent.name = payload.name
  if (props.description) scheduledEvent.description = payload.description
  if (props.privacyLevel) scheduledEvent.privacyLevel = payload.privacy_level
  if (props.status) scheduledEvent.status = payload.status
  if (props.entityType) scheduledEvent.entityType = payload.entity_type
