...
export const baseInteraction: SetupDesiredProps<
  Interaction,
  CompleteDesiredProperties<{}, true>,
  DesiredPropertiesBehavior.RemoveKey
> = {
  // This allows typescript to still check for type errors on functions below
  ...(undefined as unknown as SetupDesiredProps<
    Interaction,
    CompleteDesiredProperties<{}, true>,
    DesiredPropertiesBehavior.RemoveKey
  >),
  ...
  async respond(response, options) {
    ...
    if (this.type === InteractionTypes.ApplicationCommandAutocomplete)
    ...
    if (
      this.type === InteractionTypes.ModalSubmit &&
      type === InteractionResponseTypes.Modal
    )
      throw new Error(
        "Cannot respond to a modal interaction with another modal."
      );
  ...
  async edit(response, messageId, options) {
    if (this.type === InteractionTypes.ApplicationCommandAutocomplete)
      throw new Error("Cannot edit an autocomplete interaction.");

    // If user provides a string, change it to a response object
    if (typeof response === "string") response = { content: response };

    if (messageId) {
      return await this.bot?.helpers.editFollowupMessage(
        this.token,
        messageId,
        response
      );
    ...
    if (!this.acknowledged) {
      if (
        ...
        throw new Error(
          "This interaction has not been responded to yet and this isn't a MessageComponent or ModalSubmit interaction."
        );
  ...
  async deferUpdate(options) {
    if (this.type === InteractionTypes.ApplicationCommandAutocomplete)
      throw new Error("Cannot edit an autocomplete interaction.");
    if (this.acknowledged)
      throw new Error("Cannot defer an already responded interaction.");
    ...
    if (
      ...
      throw new Error(
        "Cannot defer to then edit an interaction that isn't a MessageComponent or ModalSubmit interaction."
      );
  ...
  async delete(messageId) {
    if (this.type === InteractionTypes.ApplicationCommandAutocomplete)
      throw new Error("Cannot delete an autocomplete interaction");

    if (messageId)
      return await this.bot?.helpers.deleteFollowupMessage(
        this.token,
        messageId
      );
    else
      return await this.bot?.helpers.deleteOriginalInteractionResponse(
        this.token
      );
  },
};
...
export function transformInteraction(
  ...
  const interaction: SetupDesiredProps<
    Interaction,
    TransformersDesiredProperties,
    DesiredPropertiesBehavior
  > = Object.create(baseInteraction);
  ...
  if (props.id && payload.id)
    interaction.id = bot.transformers.snowflake(payload.id);
  if (props.applicationId && payload.application_id)
    interaction.applicationId = bot.transformers.snowflake(
  ...
  if (props.guildLocale && payload.guild_locale)
    interaction.guildLocale = payload.guild_locale;
  ...
  if (props.user) {
    if (payload.member?.user)
      interaction.user = bot.transformers.user(bot, payload.member?.user);
    else if (payload.user)
      interaction.user = bot.transformers.user(bot, payload.user);
  }
  if (props.appPermissions && payload.app_permissions)
    interaction.appPermissions = bot.transformers.snowflake(
  ...
  if (props.channelId && payload.channel_id)
    interaction.channelId = bot.transformers.snowflake(payload.channel_id);
  ...
  if (props.entitlements && payload.entitlements)
    interaction.entitlements = payload.entitlements.map((e) =>
      bot.transformers.entitlement(bot, e)
    );
  if (
    props.authorizingIntegrationOwners &&
  ...
  if (props.attachmentSizeLimit && payload.attachment_size_limit)
    interaction.attachmentSizeLimit = payload.attachment_size_limit;
  ...
  return bot.transformers.customizers.interaction(
    bot,
    payload,
    interaction as unknown as typeof bot.transformers.$inferredTypes.interaction,
    extra
  );
}
...
> {
  ...
  if (extra?.guildId && payload.members) {
    transformed.members = new Collection(
      Object.entries(payload.members).map(([id, value]) => {
        ...
        }) as InteractionResolvedDataMember<
          TransformersDesiredProperties,
          DesiredPropertiesBehavior
        >;
  ...
  if (payload.channels) {
    transformed.channels = new Collection(
      Object.entries(payload.channels).map(([_id, value]) => {
        const channel = bot.transformers.channel(
          bot,
          value
        ) as unknown as InteractionResolvedDataChannel<
          TransformersDesiredProperties,
          DesiredPropertiesBehavior
        >;
        // We need to tell TS that we are sure the id is a bigint
  ...
  if (payload.attachments) {
    transformed.attachments = new Collection(
      Object.entries(payload.attachments).map(([key, value]) => {
        ...
        const attachment = bot.transformers.attachment(
          bot,
          value
        ) as Attachment;
  ...
  return bot.transformers.customizers.interactionDataResolved(
    bot,
    payload,
    transformed,
    {
      shardId: extra?.shardId,
      guildId: extra?.guildId
        ? bot.transformers.snowflake(extra.guildId)
        : undefined,
    }
  );
}
...
export function transformInteractionCallbackResponse(
  ...
  const response = {} as SetupDesiredProps<
    InteractionCallbackResponse,
    TransformersDesiredProperties,
    DesiredPropertiesBehavior
  >;
  ...
  if (props.interaction && payload.interaction)
    response.interaction = bot.transformers.interactionCallback(
      bot,
      payload.interaction
    );
  if (props.resource && payload.resource)
    response.resource = bot.transformers.interactionResource(
      bot,
      payload.resource,
      { shardId: extra?.shardId }
    );
...
export function transformInteractionCallback(
  ...
  const callback = {} as SetupDesiredProps<
    InteractionCallback,
    TransformersDesiredProperties,
    DesiredPropertiesBehavior
  >;
  ...
  if (props.id && payload.id)
    callback.id = bot.transformers.snowflake(payload.id);
  if (props.type && payload.type) callback.type = payload.type;
  if (props.activityInstanceId && payload.activity_instance_id)
    callback.activityInstanceId = payload.activity_instance_id;
  if (props.responseMessageId && payload.response_message_id)
    callback.responseMessageId = bot.transformers.snowflake(
      payload.response_message_id
    );
  if (props.responseMessageEphemeral && payload.response_message_ephemeral)
    callback.responseMessageEphemeral = payload.response_message_ephemeral;
  if (props.responseMessageLoading && payload.response_message_loading)
...
export function transformInteractionResource(
  ...
  const resource = {} as SetupDesiredProps<
    InteractionResource,
    TransformersDesiredProperties,
    DesiredPropertiesBehavior
  >;
  ...
  if (props.activityInstance && payload.activity_instance)
    resource.activityInstance = payload.activity_instance;
  if (props.message && payload.message)
    resource.message = bot.transformers.message(bot, payload.message, {
      shardId: extra?.shardId,
    });

  return bot.transformers.customizers.interactionResource(
    bot,
    payload,
    resource,
    extra
