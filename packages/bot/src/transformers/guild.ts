...
export function transformGuild(bot: Bot, payload: { guild: DiscordGuild } & { shardId: number }) {
  ...
  if (props.afkTimeout) guild.afkTimeout = payload.guild.afk_timeout
  if (props.approximateMemberCount) guild.approximateMemberCount = payload.guild.approximate_member_count
  if (props.approximatePresenceCount) guild.approximatePresenceCount = payload.guild.approximate_presence_count
  ...
  if (props.maxMembers) guild.maxMembers = payload.guild.max_members
  if (props.maxPresences) guild.maxPresences = payload.guild.max_presences ?? undefined
  if (props.maxVideoChannelUsers) guild.maxVideoChannelUsers = payload.guild.max_video_channel_users
  if (props.mfaLevel) guild.mfaLevel = payload.guild.mfa_level
  if (props.name) guild.name = payload.guild.name
  if (props.nsfwLevel) guild.nsfwLevel = payload.guild.nsfw_level
  if (props.preferredLocale) guild.preferredLocale = payload.guild.preferred_locale
  if (props.premiumSubscriptionCount) guild.premiumSubscriptionCount = payload.guild.premium_subscription_count
  if (props.premiumTier) guild.premiumTier = payload.guild.premium_tier
  if (props.stageInstances)
  ...
  if (props.systemChannelFlags) guild.systemChannelFlags = payload.guild.system_channel_flags
  ...
  if (props.memberCount) guild.memberCount = payload.guild.member_count ?? 0
  ...
  if (props.id) guild.id = guildId
  if (props.ownerId) guild.ownerId = bot.transformers.snowflake(payload.guild.owner_id)
