...
var _default = (pagePath, callback) => {
  ...
  const scripts = scriptsAndStyles.filter((script) =>
    script.name && script.name.endsWith(`.js`)
  );
  const styles = scriptsAndStyles.filter((style) =>
    style.name && style.name.endsWith(`.css`)
  );
  ...
  bodyScripts = bodyScripts.concat(
    scripts.filter((s) => s.rel !== `prefetch`).map((s) => {
      const scriptPath = `${__PATH_PREFIX__}/${
        JSON.stringify(s.name).slice(1, -1)
      }`;
