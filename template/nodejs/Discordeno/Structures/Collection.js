class Collection extends Map {
    ...
    _set(key, value, options = {}) {
        ...
        const v = options.removeProps ? this.properties
            .reduce((obj2, key) => {
    ...
    base(v, options = {}) {
        ...
        if (options.members) v.members = new CloneCollection({ cache: this.client.members.cache, memberGuildId: (v.id || v.guildId) });
    ...
    patch(k, v) {
        ...
        if (this.context === 'guild' && this.properties.includes('members') && v.members) {
            if (!old.members) old.members = new CloneCollection({ cache: this.client.members.cache, memberGuildId: (v.id || v.guildId) });
            v.members.forEach(x => {
...
class CloneCollection extends Map {
    ...
    values(options = {}) {
        const values = new Map([...super.keys()].map(x => this.cache._get(this.createKey(x), { transformedKey: true, raw: true })).map(x => [x.id, x]));
        return values.values();
    }

    keys() {
        const values = new Map([...super.keys()].map(x => this.cache._get(this.createKey(x), { transformedKey: true })).map(x => [x.id, x]));
        return values.keys();
    }
