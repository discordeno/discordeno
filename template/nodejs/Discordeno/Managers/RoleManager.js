...
class RoleManager {
    ...
    forge(data = {}, options = {}) {
        if (options.guild) {
            if (options.guild.roles.cache?.has(data.id)) return options.guild.roles.cache.get(data.id, { guild: options.guild });
        } else if (this.client.roles.cache?.has(data.id)) return this.client.roles.cache.get(data.id, { guild: options.guild });
