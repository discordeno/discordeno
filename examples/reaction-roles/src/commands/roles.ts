  ...
  async execute(interaction, args: CommandArgs) {
    ...
    if (args.reactions?.create) {
      ...
      if (!interaction.channelId) {
        await interaction.respond("Could not get the current channel.", {
          isPrivate: true,
        });
        return;
      }
      ...
      if (!message) {
        await interaction.respond(
          "❌ Unable to send the message correctly. Cancelling",
          { isPrivate: true }
        );
        return;
      }
      ...
      const itemCollector = new ItemCollector<
      ...
      itemCollector.onItem(async (i) => {
        ...
        if (i.data?.customId === "reactionRoles-add") {
          ...
          await i.edit({
            ...
            components: [selectRoleActionRow],
          });
          return;
        }
        ...
        if (partialRoleInfo && i.data?.customId === "reactionRoles-add-role") {
          ...
          if (!roleToAdd) {
            throw new Error(
              "Unable to get the information for the role to add"
            );
          }
        ...
        if (partialRoleInfo && i.data?.customId === "reactionRoles-add-modal") {
          ...
          await interaction.edit({
            content:
              "Use the buttons in this message to edit the message below.",
            components: [messageActionRow],
          });
        ...
        if (i.data?.customId === "reactionRoles-remove") {
          ...
          const selectMenu = removeActionRow
        ...
        if (i.data?.customId === "reactionRoles-remove-selectMenu") {
          ...
          roles = roles.filter(
            (roleInfo) => roleInfo.role.id.toString() !== roleToRemove
          );
